...
class TeethSeg():
    ...
    def snake(self):
        ...
        # Skip if phi0 is empty
        if len(SNK.phi0) == 0:
            print(f'[{time.strftime("%y%m%d-%H:%M:%S", time.localtime(time.time()))}] Skipping Step3/4 due to empty contours')
            return 0
    ...
    def tem(self):
        ...
        # Skip if Step3 was skipped (no phi_res available)
        if 'phi_res' not in self._dt:
            print(f'[{time.strftime("%y%m%d-%H:%M:%S", time.localtime(time.time()))}] Skipping Step4 - no contours from Step3')
            return 0
